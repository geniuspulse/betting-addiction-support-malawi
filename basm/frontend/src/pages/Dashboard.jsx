  ...
  useEffect(() => {
    ...
    const fetchStreak = async () => {
      try {
        ...
        if (active) {
          const count = typeof data === 'object' && data !== null && 'streak' in data ? data.streak : Number(data);
          if (!isNaN(count)) {
            setStreak(count);
          } else {
            setStreak(0);
    ...
    const fetchCheckinHistory = async () => {
      try {
        ...
        if (active && Array.isArray(history)) {
          ...
          const checkedIn = history.some(item => {
            const dateVal = item.created_date || item.createdAt || item.date;
